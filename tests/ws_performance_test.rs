use chat_server::{run_chat_server_tls, run_chat_server_tls_redis, create_test_tls_acceptor, ChatMessage};
use tokio::{
...
#[tokio::test]
async fn test_redis_high_throughput() -> WsResult<()> {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let url = format!("wss://127.0.0.1:{}", addr.port());
    ...
    let client_count = 50;  // Start with fewer clients for Redis test
    let messages_per_client = 100;
    let total_expected_messages = client_count * messages_per_client;
    ...
    for client_id in 0..client_count {
        ...
        let handle = tokio::spawn(async move {
            ...
            let send_task = tokio::spawn(async move {
                for i in 0..messages_per_client {
                    let message = format!("Redis Client {} message {}", client_id, i);
                    if let Err(_) = sender.send(Message::Text(message)).await {
                        break;
                    }
                    if i % 10 == 0 {
            ...
            let _ = send_task.await;
            let received = timeout(Duration::from_secs(65), receive_task).await
                .unwrap_or(Ok(0))
                .unwrap_or(0);
    ...
    println!("Redis High Throughput WebSocket test results:");
    println!("Clients: {}", client_count);
    println!("Messages per client: {}", messages_per_client);
    println!("Total messages sent: {}", total_expected_messages);
    println!("Total messages received: {}", total_received);
    ...
    let expected_total = total_expected_messages * (client_count - 1);
    let fan_out_ratio = total_received as f64 / total_expected_messages as f64;
    println!("Expected with perfect fan-out: {}", expected_total);
    println!("Fan-out ratio: {:.1}x", fan_out_ratio);

    assert!(messages_per_second >= 1000.0, 
            "Redis performance target not met: {:.2} msg/sec < 1000 msg/sec", 
            messages_per_second);
